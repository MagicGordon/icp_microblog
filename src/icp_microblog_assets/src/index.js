import { icp_microblog } from "../../declarations/icp_microblog";
...
async function post() { 
  ...
  try{
    await icp_microblog.post(opt, textarea.value);
    textarea.value = "";
  }catch(err){
    console.log(err);
...
async function set_name() { 
  ...
  let opt = document.getElementById("opt").value;
  try{
    await icp_microblog.set_name(opt, textarea.value);
  }catch(err){
    console.log(err);
...
async function follow() { 
  ...
  try{
    await icp_microblog.follow(Principal.fromText(textarea.value));
  }catch(err){
    console.log(err);
    error.innerText = "Set Failed!";
  }
...
async function reflesh_author_list(){
  let followed = await icp_microblog.follows();
...
async function load_posts(author) {
  ...
  switch(author) {
    case "0":
      posts = await icp_microblog.posts(1000);
      break;
    case "1":
      posts = await icp_microblog.timeline([], 1000);
      break;
    default:
      posts = await icp_microblog.timeline([Principal.fromText(author)], 1000);
  }
  console.log(posts);
